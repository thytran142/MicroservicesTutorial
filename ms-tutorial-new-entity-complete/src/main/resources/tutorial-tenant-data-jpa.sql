INSERT INTO BLC_TENANT(ID, TENANT_NAME, IDENTIFIER_TYPE, IDENTIFIER_VALUE, DEFAULT_LOCALE, ALLOWED_LOCALES)
VALUES ('5DF1363059675161A85F576D', 'Local Tenant', 'DOMAIN', 'localhost', 'en_US',
        '["en_US"]');

INSERT INTO BLC_TENANT_APPLICATION (ID, NAME, IDENTIFIER_TYPE, IDENTIFIER_VALUE, CUSTOMER_CONTEXT_ID, DEACTIVATED,
                                    DEFAULT_LOCALE, ALLOWED_LOCALES, UPDATED_TIMESTAMP, TRK_ARCHIVED, TRK_TENANT_ID)
VALUES ('1', 'Tutorial Site', 'DOMAIN', 'tutorial.localhost', '1', FALSE, 'en_US',
        '["en_US"]', '2018-01-01', FALSE, '5DF1363059675161A85F576D');
INSERT INTO BLC_TENANT_APPLICATION_CATALOG (APPLICATION_ID, CONTEXT_ID, IMPLICIT, NAME, MUTABILITY_TYPE,
                                            VISIBLE_AS_ASSIGNED, EXCLUDE_FROM_ADD, CATALOG_STATUS,
                                            ISOLATED_CATALOGS_ORDER)
VALUES ('1', '1', '10', 'TUTORIAL_CATALOG', 'CUSTOMIZABLE', TRUE, FALSE, NULL, 0);
INSERT INTO BLC_NOTIFICATION_STATE (ID, CONTAINER, ENTITY_TYPE, CHANGE_TIMESTAMP, CHANGE_TIMESTAMP_ACK, ACKED, STOPPED,
                                    ATTEMPTS, NEXT_ATTEMPT, NOTIFICATION_NAME, MESSAGE_TYPE, MESSAGE_VALUE,
                                    NOTIFICATION_STATES_ORDER)
VALUES ('1', '1', 'com.broadleafcommerce.tenant.provider.jpa.domain.JpaTenantApplication', '2018-01-01', NULL, FALSE, FALSE,
        0, NULL, 'PERSISTENCE', NULL, NULL, 0);

INSERT INTO BLC_TENANT_CATALOG (ID, OWNING_APPLICATION, CATALOG_NAME, HIDDEN, LEVEL, UPDATED_TIMESTAMP, CATALOG_LOCALE,
                                TRK_ARCHIVED, TRK_TENANT_ID)
VALUES ('1', NULL, 'TUTORIAL_CATALOG', FALSE, 1, '2018-01-01', 'en_US', FALSE, '5DF1363059675161A85F576D');
INSERT INTO BLC_NOTIFICATION_STATE (ID, CONTAINER, ENTITY_TYPE, CHANGE_TIMESTAMP, CHANGE_TIMESTAMP_ACK, ACKED, STOPPED,
                                    ATTEMPTS, NEXT_ATTEMPT, NOTIFICATION_NAME, MESSAGE_TYPE, MESSAGE_VALUE,
                                    NOTIFICATION_STATES_ORDER)
VALUES ('3', '1', 'com.broadleafcommerce.tenant.provider.jpa.domain.JpaTenantCatalog', '2018-01-01', NULL, FALSE, FALSE, 0,
        NULL, 'PERSISTENCE', NULL, NULL, 0);

INSERT INTO BLC_TENANT_CATALOG (ID, OWNING_APPLICATION, CATALOG_NAME, HIDDEN, LEVEL, UPDATED_TIMESTAMP, CATALOG_LOCALE,
                                TRK_ARCHIVED, TRK_TENANT_ID)
VALUES ('10', '1', 'Tutorial Site''s Catalog', TRUE, 2, '2018-01-01', 'en_US', FALSE, '5DF1363059675161A85F576D');
INSERT INTO BLC_TENANT_CATALOG_REF (ID, NAME, PARENT_CATALOG_ID, CHILD_CATALOG_ID, MUTABILITY_TYPE, INCLUSION_FILTERS,
                                    EXCLUSION_FILTERS, PARENTS_ORDER)
VALUES ('1', 'TUTORIAL_CATALOG', '1', '10', 'CUSTOMIZABLE', NULL, NULL, 0);
INSERT INTO BLC_NOTIFICATION_STATE (ID, CONTAINER, ENTITY_TYPE, CHANGE_TIMESTAMP, CHANGE_TIMESTAMP_ACK, ACKED, STOPPED,
                                    ATTEMPTS, NEXT_ATTEMPT, NOTIFICATION_NAME, MESSAGE_TYPE, MESSAGE_VALUE,
                                    NOTIFICATION_STATES_ORDER)
VALUES ('4', '10', 'com.broadleafcommerce.tenant.provider.jpa.domain.JpaTenantCatalog', '2018-01-01', NULL, FALSE, FALSE, 0,
        NULL, 'PERSISTENCE', NULL, NULL, 0);